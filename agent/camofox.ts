export interface Tabs {
	create(url: string, sessionKey: string): Promise<Record<string, unknown>>;
...
export function createCamofox(
	...
	const request = async (
		...
		const res = await fetcher(camofoxUrl + path, {
			method,
	...
	return {
		create: (url, sessionKey) =>
			request("POST", "/tabs", { userId, sessionKey, url }),
		...
		navigate: (id, url) =>
			request("POST", `/tabs/${id}/navigate`, { userId, url }),
...
export async function wakeBrowser(
	...
	try {
		await fetcher(`${camofoxUrl}/health`);
	} catch {}
}
export async function createTabWithRetry<T>(
	...
	for (let attempt = 1; attempt <= 3; attempt++) {
		try {
			...
			if (!message.includes("503") && !message.includes("session_expired"))
				throw error;
			if (attempt < 3) await wake();
		}
	}
	throw lastError;
}
